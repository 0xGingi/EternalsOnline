const FISHES = require('../../config/agility.json');
    ...
    async execute(message, args, commandName) {
		...
		if (message.content.toLowerCase() && this.info.names.some(name => commandName === name)) {
            ...
            var embed = new EmbedBuilder()
                .setColor(0x0099FF)
                .setTitle('Agility Course List')
                .setDescription('Here are all the agility courses:')
                .setTimestamp();


                current.sort((a, b) => a.level - b.level).forEach(fish => {
                embed.addFields(
                    { name: fish.name, value: `Agility Level: ${fish.level}\nArea: ${fish.area}\nXP: ${fish.xp}`, inline: true }
                );
            });
