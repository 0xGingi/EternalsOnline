const BARS = require('../../config/smelt.json'); // Make sure this path is correct
    ...
    async execute(message, args, commandName) {
        if (message.content.toLowerCase() && this.info.names.some(name => commandName === name)) {
            ...
            var embed = new EmbedBuilder()
                .setColor(0x0099FF)
                .setTitle('Bar List')
                .setDescription('Here are all the bars you can smelt:')
                .setTimestamp();
                ...
                embed.addFields(
                    { name: bar.name, value: `Smithing Level: ${bar.level}\nOres Needed:\n${oreList}\nXP: ${bar.xp}`, inline: true }
                );
            });
