...
module.exports = {
    ...
    async execute(message, args, commandName) {
        ...
        if (!memberToPromote) {
            return message.reply(`${EMOJICONFIG.no} Please mention a user to demote.`);
        }
        ...
        if (!squad.officer.some(officer => officer.id === memberToPromote.id)) {
            return message.reply(`${EMOJICONFIG.no} This member isn't an officer.`);
        }

        var officerIndex = squad.officer.findIndex(officer => officer.id === memberToPromote.id);
        if (officerIndex !== -1) {
            squad.officer.splice(officerIndex, 1);
        }
        await squad.save();

        return message.reply(`${EMOJICONFIG.yes} ${memberToPromote.username} has been demoted from officer.`);
    }
    },
info: {
    names: ['demote'],
