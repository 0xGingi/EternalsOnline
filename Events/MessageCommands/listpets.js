const FISHES = require('../../config/pets.json');
    ...
    async execute(message, args, commandName) {
		...
		if (message.content.toLowerCase() && this.info.names.some(name => commandName === name)) {
            ...
            for (let i = 0; i < FISHES.length; i += 12) {
                ...
                .setColor(0x0099FF)
                .setTitle(`${EMOJICONFIG.hellspawn} Pet List`)
                .setDescription('Here are all the pets you can catch:')
                .setTimestamp();
                ...
                current.forEach(fish => {
                    embed.addFields(
                        { 
                            name: `${fish.name}`, 
                            value: `Attack: ${fish.attack}\nDefense: ${fish.defense}\nHealth: ${fish.health}\nSpecial Attack: ${fish.specialMove}\nChance: ${Math.round(fish.chance * 100)}%`, 
                            inline: true 
                        }
                    );
                });
