...
const CONFIGITEM = require('../../config/topbox.json')
...
module.exports = {
    ...
    if (!playerStats) return message.reply(`${EMOJICONFIG.no} you are not a player ! : ${inlineCode('@FlipMMO start')}`);
        ...
        if (!balance) return message.reply(`${EMOJICONFIG.no} you are not a player ! : ${inlineCode('@FlipMMO start')}`);
            ...
            if (playerStats.player.other.topggbox == 0) return message.reply(`${EMOJICONFIG.no} You don't have any top.gg vote reward boxes to open! \n Vote At https://top.gg/bot/1157454837861056552/vote`);
            else {
            ...
            async function randomitem(){
                ...
                playerStats.player.other.topggbox -= 1;
            ...
            async function openAllBoxes() {
                if(playerStats.player.other.topggbox > 0){
                    let boxesOpened = 0;
                    let itemsFound = [];
                    while (playerStats.player.other.topggbox > 0) {
                    ...
                    .setDescription(`${EMOJICONFIG.yes} **All top.gg boxes opened!**\n${itemsFound.join('\n')}`)
                    .setTimestamp();
                    return message.reply({ embeds: [itemEmbed] });
                } else {
                    return message.reply(`${EMOJICONFIG.no} You don't have any top.gg vote reward boxes to open!\n Vote At https://top.gg/bot/1157454837861056552/vote`);
                }
            }
            ...
            if (args[0] && args[0].toLowerCase() === 'all') {
                ...
                var itemEmbed = new EmbedBuilder()
                    ...
                    .setDescription(`${EMOJICONFIG.yes} **Top.gg Box open!**\n${result.nname}`)
                    .setTimestamp()
                return message.reply({ embeds: [itemEmbed]})
            }
