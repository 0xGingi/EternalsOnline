...
module.exports = {
    ...
    async execute(message, args, command) {
        ...
        if (!args[0]) {
            ...
            if (dungeonFloor == 0 && playerStats.player.slayer.level >= 10) {
                 dungeonFloor = Math.min(Math.floor(playerStats.player.slayer.level / 10), 12);
            }
        }        if (dungeonFloor > 12) {
            return message.reply('The maximum Slayer Dungeon floor is 12.');
        }
        playerStats.player.other.slayerdungeonFloor = dungeonFloor;
        await playerStats.save();
        const dungeonName = 'slayerdungeon';
        const dungeonDisplay = 'Slayer Dungeon';
        ...
        if (playerStats.player.slayer.level < requiredSlayerLevel) {
            return message.reply(`You need a Slayer level of ${requiredSlayerLevel} to access floor ${dungeonFloor}.`);
        }
        const task = `Slayer Dungeon Floor ${dungeonFloor}`;
        
        if (playerStats.player.slayer.task.monster != task) {
        ...
        if (!balance) return message.reply(`${EMOJICONFIG.no} you are not a player ! : ${inlineCode('@Eternals start')}`);
            ...
            const dungeonConfigtrash = require(`../../config/slayerdungeon.${dungeonFloor}.json`);
            const monstersConfig = dungeonConfigtrash;
            ...
            async function battle(MAXATK_PLAYER, MAXATK_MONSTER, HEALTH_PLAYER, HEALTH_MONSTER, DEFENSE_MONSTER, DODGEPLAYER, CRITPLAYER, MAXXP, DEFENSE_PLAYER){
                ...
                while(HEALTH_PLAYER != 0 || HEALTH_MONSTER != 0){
                   // console.log(HEALTH_PLAYER)
                    ...
                    // =========== PLAYER WIN ===========
                        ...
                        var slayerxp = selectedMonster.slayerxp;
                ...
                return {inparty, sharedXpPercentage};
                       ...
                       // playerStats.player.cooldowns = playerStats.player.cooldowns || {};
                        ...
                        playerStats.player.slayer.xp += slayerxp;
                        await stats.save();
                        await playerStats.save();
                        await balance.save();
                        ...
                        var progressDisplay = playerStats.player.slayer.task.kills;
                        var killsNeeded = playerStats.player.slayer.task.neededKills;
                        var killsLeft = playerStats.player.slayer.task.neededKills - playerStats.player.slayer.task.kills;
                        
                        var progressMessage = playerStats.player.slayer.task.neededKills === playerStats.player.slayer.task.kills 
                            ? `You have completed the ${dungeonDisplay} Dungeon!` 
                            : `You have ${progressDisplay} / ${killsNeeded} kills. You need ${killsLeft} more to complete your task`;
                        ...
                        var battleEmbed = new Discord.EmbedBuilder()
                            ...
                            .addFields(
                                ...
                                { name: `**${EMOJICONFIG.scroll4} STATS :**\n`, value: `**YOU WIN**\n${EMOJICONFIG.yes} And get: **${inlineCode(numStr(randomxp))}** ${EMOJICONFIG.xp} - **${inlineCode(numStr(slayerxp))} Slayer XP** - **${inlineCode(numStr(randomcoin))}** ${EMOJICONFIG.coin} - **${inlineCode(numStr(boxwin))}** ${EMOJICONFIG.coinchest} `, inline: false },
                                { name: `**${EMOJICONFIG.paper} Slayer Task Progression :**\n`, value: progressMessage},
