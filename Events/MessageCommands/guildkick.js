  ...
  //  const commandName = args.shift().toLowerCase();
    ...
    if (!playerStats) return message.reply(`${EMOJICONFIG.no} you are not a player ! : ${inlineCode('@FlipMMO start')}`);
        ...
        if (!squad) return message.reply(`${EMOJICONFIG.no} you are not in a guild...`)
            ...
            if(user.id != squad.leader[0]) return message.reply(`${EMOJICONFIG.no} you are not the leader of the Guild: ${inlineCode(squad.squadName)}`); 
            ...
            if(playerInSquad(playerBan, playerStats)){
                ...
                var banMessage = new EmbedBuilder()
                    .setColor('#2f3136')
                    .setTitle(`${EMOJICONFIG.hammer2} Kick ${playerBan.pseudo} from ${inlineCode(squad.squadName)}`)
                    .setDescription(`Do you want to kick "${playerBan.pseudo}" from your Guild: ${inlineCode(squad.squadName)}`)
                    .setTimestamp();
                ...
                collector.on('collect', async interaction => {
                    if (interaction.customId == 'yes') {
                        ...
                        await interaction.reply({ content: `${EMOJICONFIG.yes} ${playerBan.pseudo} has been successfully kicked from your Guild`, ephemeral: true });
                    };
                    if (interaction.customId == 'no') {
                        await interaction.reply({ content: `${EMOJICONFIG.no}You have cancelled the kick of ${playerBan.pseudo}`, ephemeral: true });
                    }
                });
            } else return message.reply(`${EMOJICONFIG.no} The user mentioned is not in the same Guild as you...`) 
        };
    };
};
},
