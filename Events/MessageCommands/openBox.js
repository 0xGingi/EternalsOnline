...
const CONFIGITEM = require('../../config/commonbox.json')
...
module.exports = {
    ...
    if (!playerStats) return message.reply(`${EMOJICONFIG.no} you are not a player ! : ${inlineCode('@Eternals start')}`);
        ...
        if (!balance) return message.reply(`${EMOJICONFIG.no} you are not a player ! : ${inlineCode('@Eternals start')}`);
            ...
            if (playerStats.player.other.box == 0) return message.reply(`${EMOJICONFIG.no} You don't have any boxes to open`);
            else {
            ...
            async function openAllBoxes() {
                if(playerStats.player.other.box > 0){
                    let boxesOpened = 0;
                    let itemsFound = [];
                    while (playerStats.player.other.box > 0) {
                    ...
                    .setDescription(`${EMOJICONFIG.yes} **All boxes opened!**\n${itemsFound.join('\n')}`)
                    .setTimestamp();
            ...
            if (args[0] && args[0].toLowerCase() === 'all') {
                ...
                var itemEmbed = new EmbedBuilder()
                    ...
                    .setDescription(`${EMOJICONFIG.yes} **Box open!**\n${result.nname}`)
                    .setTimestamp()
                return message.reply({ embeds: [itemEmbed]})
            }
