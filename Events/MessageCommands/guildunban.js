  ...
  //  const commandName = args.shift().toLowerCase();
    ...
    if (!playerStats) return message.reply(`${EMOJICONFIG.no} you are not a player ! : ${inlineCode('@Eternals start')}`);
        ...
        if (!squad) return message.reply(`${EMOJICONFIG.no} you are not in a guild...`)
            ...
            let isIssuerOfficer = squad.officers.includes(user.id) || user.id === squad.leader[0];
            let isTargetLeader = userBan.id === squad.leader[0];
            let isTargetOfficer = squad.officers.includes(userBan.id);
            ...
            if (isIssuerOfficer && (isTargetLeader || isTargetOfficer)) {
                return message.reply(`${EMOJICONFIG.no} you cannot ban the leader or another officer of the Guild: ${inlineCode(squad.squadName)}`);
            }
            ...
            let playerBan = await PLAYERDATA.findOne({ userId: userBan.id });
                ...
                var banMessage = new EmbedBuilder()
                    .setColor('#2f3136')
                    .setTitle(`${EMOJICONFIG.hammer2} Unban ${playerBan.pseudo} from ${inlineCode(squad.squadName)}`)
                    .setDescription(`Do you want to unban "${playerBan.pseudo}" from your Guild: ${inlineCode(squad.squadName)}`)
                    .setTimestamp();
...
collector.on('collect', async interaction => {
    if (interaction.customId == 'yes') {
        ...
        if (!squad.banned.some(bannedUser => bannedUser.id === userBan.id)) {
            return message.reply(`${EMOJICONFIG.no} ${playerBan.pseudo} is not banned from your Guild`);
        }

        var index = squad.banned.findIndex(bannedUser => bannedUser.id === userBan.id);
        if (index !== -1) {
            squad.banned.splice(index, 1);
        }

        await squad.save();

        await interaction.reply({ content: `${EMOJICONFIG.yes} ${playerBan.pseudo} has been successfully unbanned from your Guild`, ephemeral: true });
    };
